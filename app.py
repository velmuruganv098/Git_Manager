...
"Previous revision" and "New revision" double as real branch names:

    UPLOAD (local -> GitHub):
        Previous revision = an existing branch to compare/branch off
                             (e.g. V0.006) - leave blank for a first push
        New revision       = the branch to create and push to
                             (e.g. V0.007) - must not already exist

    PULL (GitHub -> local):
        New revision       = the existing remote branch to fetch and
                             replace the local copy with (e.g. V0.006)
        Previous revision  = just a label for what local was called
                             before the replace (optional, note only)
...
def require_repo_ready(path):
    if not git_manager.is_repo(path):
        raise ValueError(
            "This folder is not a git repository yet. Go to the Status tab "
            "and click 'Prepare repository' first."
        )


def note_already_exists(path, cfg, new_rev):
    """Returns the existing difference-note filename if `new_rev` has
    already been used as a 'TO' revision for this project locally."""
    notes_dir = os.path.join(path, cfg.get("revision_notes_folder", "Revision_Notes"))
    if not os.path.isdir(notes_dir):
        return None
    suffix = f"_TO_{safe_filename_part(new_rev)}.txt".lower()
...
def next_revision_label(label):
    m = re.match(r"^(.*?)(\d+)$", label.strip())
    if m:
        prefix, num = m.groups()
        width = len(num)
        return f"{prefix}{str(int(num) + 1).zfill(width)}"
    return label + "-next"


def safe_filename_part(label):
    """Branch names can legally contain '/' (e.g. 'dev/can1-v0.004') - fine
    for git, but a slash in a filename is a path separator on disk and
    would try to write into a subfolder that doesn't exist. This swaps
    anything filesystem-unsafe for '-' ONLY for building file/folder
    names; the real branch name (with the slash) is still what's used for
    every actual git operation and shown in the note's own text."""
...
def environment():
    return jsonify({"ok": True, "git_available": git_manager.is_git_available()})
...
def status():
    ...
    info = {"ok": True, "repo_ready": is_repo, "local_project_path": path}
    if is_repo:
...
def init_or_clone():
    ...
    not exist yet locally, `git init` + attach the remote if it exists but
    isn't a repo yet, or - if it's already a repo - correct the remote URL
    in case an earlier run of this app saved a malformed one."""
    cfg = config_manager.load_config()
    try:
        ...
        clean_url = git_manager.build_https_url(owner, repo, None)

        if not os.path.exists(path) or not os.listdir(path):
            git_manager.clone_repo(url, path)
            action = "Cloned the repository into this folder."
        elif not git_manager.is_repo(path):
            git_manager.init_repo(path)
            git_manager.set_remote_url(path, clean_url)
            action = "Turned this existing folder into a git repository linked to your remote."
        else:
            current = git_manager._scrub_token(git_manager.get_remote_url(path))
            if current != clean_url:
                git_manager.set_remote_url(path, clean_url)
                action = f"Already a git repository - corrected the remote URL to {clean_url}."
            else:
                action = "Already a git repository, remote URL looks correct."

        activity_log.append_activity({
            "operation": "PREPARE REPOSITORY", "result": "SUCCESS",
            "details": action, "local": path, "github": f"{owner}/{repo}",
        })
        return jsonify({"ok": True, "action": action})
    except (ValueError, git_manager.GitError) as e:
        activity_log.append_activity({"operation": "PREPARE REPOSITORY", "result": "FAILED", "details": str(e)})
        return error_response(str(e))


@app.route("/api/branches", methods=["GET"])
def branches():
    """List branches that exist on the remote, for the New/Previous
    revision pickers in the UI. Returns an empty list rather than an error
    whenever anything isn't ready yet, since this is just used to help
    autocomplete - it should never block the rest of the page."""
    cfg = config_manager.load_config()
    try:
        path = require_project(cfg)
        owner, repo = require_github(cfg)
        ...
        try:
            ...
            pass
        return jsonify({"ok": True, "branches": git_manager.remote_branch_names(path)})
    except ValueError:
        return jsonify({"ok": True, "branches": []})
...
def upload_scan():
    ...
    try:
        ...
        owner, repo = require_github(cfg)
        token = cfg.get("github_token") or None
        require_repo_ready(path)

        push_branch = cfg.get("new_revision", "").strip()
        if not push_branch:
            raise ValueError("Enter the branch name to push this revision to (New revision).")
        base_branch = cfg.get("previous_revision", "").strip()

        try:
            git_manager.fetch(path, token=token, owner=owner, repo=repo)
        except git_manager.GitError:
            pass  # offline / first push - fall back to whatever is available locally

        # The push target must be a genuinely new branch - this is the
        # "reject a reused version name" check.
        local_clash = note_already_exists(path, cfg, push_branch)
        if local_clash:
            raise ValueError(
                f"Revision '{push_branch}' is already used ({local_clash} already exists "
                f"in {cfg.get('revision_notes_folder', 'Revision_Notes')}/). "
                f"Choose a different New revision / branch name."
            )
        if git_manager.ref_exists(path, f"origin/{push_branch}"):
            raise ValueError(
                f"Branch '{push_branch}' already exists on GitHub. "
                f"Choose a new, unused branch name for New revision."
            )

        base_ref = None
        if base_branch:
            base_ref = f"origin/{base_branch}"
            if not git_manager.ref_exists(path, base_ref):
                raise ValueError(
                    f"Branch '{base_branch}' (Previous revision) was not found on "
                    f"GitHub after fetching. Check the branch name, or leave "
                    f"Previous revision blank for a first upload."
                )
        elif git_manager.ref_exists(path, "HEAD"):
            base_ref = "HEAD"  # no explicit base picked - fall back to local HEAD

        if base_ref:
            old_archive = git_manager.archive_ref_to_dir(path, base_ref)
        else:
            old_archive = tempfile.mkdtemp(prefix="lgm_empty_")
        ...
        meta = {
            ...
            "from_rev": base_branch or "INITIAL",
            "to_rev": push_branch,
            "branch": push_branch,
            "direction": "LOCAL -> GITHUB",
            "user_notes": cfg.get("revision_notes", ""),
            "source_label": path,
            "dest_label": f"{owner}/{repo} (branch: {push_branch})",
        }
        note_text = diff_note.build_diff_note(comparison, meta)
        filename = (
            f"{cfg.get('diff_note_prefix', 'DIFFERENCE')}_"
            f"{safe_filename_part(meta['from_rev'])}_TO_{safe_filename_part(meta['to_rev'])}.txt"
        )

        _LAST_SCAN["upload"] = {
            "comparison": comparison, "meta": meta, "filename": filename,
            "note_text": note_text, "base_branch": base_branch, "push_branch": push_branch,
        }
...
def upload_confirm():
    ...
    try:
        ...
        # Point HEAD at push_branch (creating it from base_branch's tip if
        # it doesn't exist yet) WITHOUT touching any files on disk - what's
        # already in the folder is exactly what gets committed.
        base_ref = f"origin/{base_branch}" if base_branch else None
        git_manager.prepare_branch_for_commit(path, push_branch, base_ref)
        ...
        git_manager.push(path, push_branch, token=token, owner=owner, repo=repo)

        # Verify: local HEAD should now equal the remote branch tip.
        git_manager.fetch(path, token=token, owner=owner, repo=repo)
        local_sha = git_manager.current_commit(path)
        remote_sha = git_manager.current_commit(path, f"origin/{push_branch}")
        ...
        config_manager.save_config({
            "previous_revision": push_branch,
            "new_revision": next_revision_label(push_branch),
        })
...
def pull_scan():
    ...
    try:
        ...
        source_branch = cfg.get("new_revision", "").strip()
        if not source_branch:
            raise ValueError("Enter the branch name to pull from (New revision).")
        ...
        if not git_manager.ref_exists(path, remote_ref):
            raise ValueError(f"Branch '{source_branch}' was not found on the remote after fetch.")
        ...
        meta = {
            "project_name": project_meta(cfg),
            "from_rev": from_rev,
            "to_rev": source_branch,
            "branch": source_branch,
            "direction": "GITHUB -> LOCAL",
            "user_notes": cfg.get("revision_notes", ""),
            "source_label": f"{owner}/{repo} (branch: {source_branch})",
            "dest_label": path,
        }
        note_text = diff_note.build_diff_note(comparison, meta)
        filename = (
        ...
        _LAST_SCAN["pull"] = {
            "comparison": comparison, "meta": meta, "filename": filename,
            "note_text": note_text, "source_branch": source_branch,
        }
...
def pull_confirm():
    ...
    try:
        ...
        git_manager.pull_reset_hard(
            path, source_branch, token=token, owner=owner, repo=repo,
            protect_dirs=[backup_folder_name + "/"],
        )

        # Write the difference note AFTER the replace, so the reset/clean
        # step can't remove it either. If the same note filename already
        # exists (e.g. this branch was pulled before), keep both copies
        # rather than silently overwriting the older one.
        notes_dir = os.path.join(path, notes_folder_name)
        os.makedirs(notes_dir, exist_ok=True)
        note_path = os.path.join(notes_dir, scan["filename"])
        if os.path.exists(note_path):
            stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            base, ext = os.path.splitext(scan["filename"])
            note_path = os.path.join(notes_dir, f"{base}_{stamp}{ext}")
        with open(note_path, "w", encoding="utf-8") as f:
            f.write(scan["note_text"])

        remote_sha = git_manager.current_commit(path, f"origin/{source_branch}")
        local_sha = git_manager.current_commit(path)
        verified = bool(local_sha and remote_sha and local_sha == remote_sha)
        ...
        record = {
            ...
            "difference_note": os.path.basename(note_path),
            "local": path,
            "github": f"{owner}/{repo}",
        }
        activity_log.append_activity(record)
        ...
        config_manager.save_config({
            "previous_revision": source_branch,
            "new_revision": next_revision_label(source_branch),
        })
...
def handle_unexpected(e):
    if isinstance(e, HTTPException):
        ...
        # being reported as a scary 500 "unexpected error".
        return e
    traceback.print_exc()
