...
def remote_branch_names(path, remote="origin"):
    """Plain branch names on the remote (e.g. 'V0.006'), without the
    'origin/' prefix, for populating a picker in the UI."""
    branches = list_branches(path)
    prefix = remote + "/"
    names = sorted({b[len(prefix):] for b in branches["remote"] if b.startswith(prefix)})
    return names
...
def prepare_branch_for_commit(path, push_branch, base_ref=None):
    """Point HEAD at push_branch (creating the branch ref from base_ref if
    it doesn't exist yet) and sync the *index* to that branch's tree -
    WITHOUT touching any working-tree files.

    This is what lets a local folder that's already been edited by hand
    (no git checkout involved) be diffed and committed against a specific
    existing branch (e.g. branching V0.007 off V0.006) without git ever
    overwriting what's actually on disk."""
    branches = list_branches(path)
    branch_exists_locally = push_branch in branches["local"]

    if not branch_exists_locally:
        if base_ref and ref_exists(path, base_ref):
            _run(["branch", push_branch, base_ref], cwd=path)
        elif ref_exists(path, "HEAD"):
            _run(["branch", push_branch], cwd=path)
        # else: no commits anywhere yet in this repo - the branch will be
        # created by the commit itself once HEAD is pointed at it below.

    _run(["symbolic-ref", "HEAD", f"refs/heads/{push_branch}"], cwd=path)

    if ref_exists(path, push_branch):
        _run(["read-tree", "--reset", push_branch], cwd=path)
...
def pull_reset_hard(path, branch, remote="origin", token=None, owner=None, repo=None,
    ...
    Deliberately does NOT use `git checkout` to switch onto `branch` first:
    checkout refuses when untracked local files would be overwritten,
    which is exactly the everyday case here (a local folder edited by hand
    outside of git). `git branch` + `symbolic-ref` point HEAD at the branch
    without touching any files, and `reset --hard` (unlike checkout) does
    the working-tree overwrite unconditionally - which is the whole point
    of a "replace local with remote" operation the user has already
    confirmed and already has a backup for.
    ...
    def do_pull():
        ...
        if branch not in branches["local"]:
            if ref_exists(path, remote_ref):
                _run(["branch", branch, remote_ref], cwd=path)
            elif ref_exists(path, "HEAD"):
                _run(["branch", branch], cwd=path)
        _run(["symbolic-ref", "HEAD", f"refs/heads/{branch}"], cwd=path)
        _run(["reset", "--hard", remote_ref], cwd=path)
        clean_args = ["clean", "-fd"]
