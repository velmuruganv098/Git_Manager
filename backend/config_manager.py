"""
Persistent configuration for the Local GitHub Manager.
...
DEFAULT_CONFIG = {
    "local_project_path": "",      # e.g. C:\\Projects\\Zitto_MB_V1
    ...
    # "previous_revision" / "new_revision" double as REAL git branch names:
    #   Upload:  previous_revision = existing branch to compare/branch off
    #             (blank = first push, no base yet)
    #            new_revision      = branch to create and push to (must be new)
    #   Pull:    new_revision      = existing remote branch to fetch and
    #             replace the local copy with
    #            previous_revision = just a label for what local was called
    #             before the replace (optional, for the note only)
    "previous_revision": "",
    "new_revision": "",
    "revision_notes": "",
...
def load_config():
    ...
    # Migrate old configs that still have a single "branch" field from an
    # earlier version of this app: use it to seed both revision fields if
    # they haven't been set to anything yet.
    old_branch = cfg.pop("branch", None)
    if old_branch and not cfg.get("previous_revision") and not cfg.get("new_revision"):
        cfg["previous_revision"] = old_branch
    cfg.pop("github_path", None)  # no longer used - dropped from the UI
