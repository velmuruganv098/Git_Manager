...
function showView(name) {
  ...
  if (name === "upload" || name === "pull" || name === "settings") loadBranchOptions();
}
...
const CONFIG_FIELDS = [
  ...
  "github_token", "previous_revision",
...
async function loadBranchOptions() {
  ...
  list.innerHTML = (data.branches || []).map(b => `<option value="${escapeHtml(b)}">`).join("");
}
